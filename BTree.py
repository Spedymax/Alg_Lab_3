class BTreeNode:
    ...
    def update(self, k, value):
        ...
        if i < len(self.keys) and k == self.keys[i][0]:
            ...
            return self.children[i].update(k, value)
...
class BTree:
    ...
    def is_exists(self, key):
        return self.is_exists_recursive(self.root, key)

    def is_exists_recursive(self, node, key):
        i = 0
        ...
        if i < len(node.keys) and key[0] == node.keys[i][0]:
            ...
            return self.is_exists_recursive(node.children[i], key)
    ...
    def save(self, node):
        for i in range(0, len(node.keys)):
            if not node.leaf:
                self.save(node.children[i])
            self.btree_data.append(node.keys[i])
        if not node.leaf:
            self.save(node.children[-1])

    def update(self, k, value):
        self.root.update(k, value)
    ...
    def search(self, node, key):
        ...
        while left <= right:
            ...
            mid = (left + right) // 2
            self.comparison_counter += 1
            if node.keys[mid][0] == key:
                return node.keys[mid][1]
            elif node.keys[mid][0] < key:
                self.comparison_counter += 1
                left = mid + 1
            else:
                self.comparison_counter += 1
                right = mid - 1
        if not node.leaf:
            return self.search(node.children[left], key)
        else:
            return None
